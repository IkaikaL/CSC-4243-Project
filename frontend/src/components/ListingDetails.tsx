...
const ListingDetails = (props: Props) => {
	...
	const priceCategories = ["for sale", "housing", "services"];

	const wageCategories = ["jobs"];
	...
	const priceSpace = () => {
		...
		return <Typography color='white'>{priceStr}</Typography>;
	};
	...
	const detailTable = () => {
		...
		if (priceCategories.includes(mainCategory)) {
			extraRows = [
				<TableRow>
					<TableCell>
						<Typography color='white' sx={{ height: "10px" }}>
							Condition
						</Typography>
					</TableCell>
					<TableCell>
